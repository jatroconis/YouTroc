...
/**
 * Feature-edge mapping: turns the pure domain [Video] type into the
 * presentation [VideoCardUi]. The Spanish user-facing copy ("N vistas ·
 * fecha") lives here — never in `:core:domain` (which stays free of
 * localized strings) nor in `:core:ui` (which stays ignorant of the domain).
 *
 * [SearchMetaFormatter] is a deliberate duplicate of `:feature:catalog`'s
 * `CatalogMetaFormatter` (design's rule-of-three deferral): consolidating
 * into a shared `:core:ui` helper is deferred until a 3rd consumer (e.g.
 * `:feature:video`) needs the same formatting.
 */

/** Maps a domain [Video] into a [VideoCardUi] with a Spanish [SearchMetaFormatter] meta line. */
internal fun Video.toVideoCardUi(): VideoCardUi = VideoCardUi(
    id = id.value,
    thumbnailUrl = thumbnailUrl,
    title = title,
    channel = channelName,
    meta = SearchMetaFormatter.format(viewCount, publishedText),
)
